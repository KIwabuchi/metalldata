...
struct ProcessDataMG {
  ...
  void commEdgeSrcCheck(std::string src, std::string tgt, std::size_t idx) {
    void commEdgeSrcCheck(std::string src, std::string tgt, std::size_t idx) {
      ...
      struct metall_graphKeys
        ...
        struct metall_graphKeys
          ...
          std::string_view nodes_key() const { return std::get<0>(*this); }
          std::string_view edges_source_key() const {
            return std::get<1>(*this);
          }
          std::string_view edges_target_key() const {
    ...
    metall_json_lines::accessor_type get_key(
        metall_json_lines::accessor_type val, std::string_view key) {
  ...
  struct mg_count_summary : std::tuple<std::size_t, std::size_t> {
    struct mg_count_summary : std::tuple<std::size_t, std::size_t> {
      ...
      boost::json::object asJson() const {boost::json::object asJson()
                                              const {boost::json::object res;
...
static void
create_new(metall_manager_type &manager, ygm::comm &comm,
           std::string_view node_key, std::string_view edge_src_key,
  ...
  key_store_type &vec = checked_deref(
      mgr.construct<key_store_type>(keys_location_suffix)(mgr.get_allocator()),
      ERR_CONSTRUCT_KEYS);
...
auto edgeAction = [&selectedKeys, edgeSrcKeyTxt = edgeSrcKey(),
                   edgeTgtKeyTxt = edgeTgtKey()](
                      std::size_t                               pos,
                      const xpr::metall_json_lines::value_type &val) -> void {
  commEdgeSrcCheck(to_string(get_key(val, edgeSrcKeyTxt)),
                   to_string(get_key(val, edgeTgtKeyTxt)), pos);
};
}
* /
...
std::vector<std::size_t> kcore(std::vector<filter_type> nfilt,
                               std::vector<filter_type> efilt, int max_kcore) {
  ...
  for (int kcore = 1; kcore <= max_kcore; ++kcore) {
                        ...
                        while (true) {
                          ...
                          adj_set.for_all([kcore, &adj_set, &locally_pruned,
                                           &kcore_table,
                                           this](const std::string     &vert,
                          ...
                          const auto global_pruned =
                              comm().all_reduce_sum(locally_pruned);
...
size_t bfs(std::vector<filter_type> nfilt, std::vector<filter_type> efilt,
  ...
  for (size_t level = 0;; ++level) {
                        ...
                        adj_list.for_all([level, &count, &adj_list](
                          ...
                          auto &current_lv =
                              bfs_comp_mg::ptr->level_table.at(v);
                          ...
                          for (const auto &n : adj) {
                            adj_list.async_visit(
                                n,
                                [](std::string v, std::vector<std::string> &adj,
                                   size_t level) {
                                  auto &current_lv =
                                      bfs_comp_mg::ptr->level_table.at(v);
                                  if (current_lv == -1) {
...
void dump(const std::string_view &prefix_path) {
      ...
      std::string(prefix_path) + "-node-" + std::to_string(comm().rank());
  {
                        ...
                        for (std::size_t i = 0; i < nodelst.local_size(); ++i) {
                          ofs << nodelst.at(i) << "\n";
                          // std::cerr << nodelst.at(i) << std::endl;
                        }
  }
      ...
      std::string(prefix_path) + "-edge-" + std::to_string(comm().rank());
  {
                        ...
                        for (std::size_t i = 0; i < edgelst.local_size(); ++i) {
                          ofs << edgelst.at(i) << "\n";
                          // std::cerr << edgelst.at(i) << std::endl;
                        }
  }
  comm().cf_barrier();
}
